...
std::string SAESd(std::string input, int rounds, std::vector<std::vector<char>> roundKeys);
...
int main(int argc, char* argv[])
{
	...
	std::cout << SAESd(input, rounds, roundKeys);
...
std::string SAESd(std::string input, int rounds, std::vector<std::vector<char>> roundKeys)
{
    ...
    {                
        ...
        for (int j = 0; j < inputAsBlocks.size(); j++)
            for (int c = 0; c < 3; c++)
                for (int r = 0; r < 3; r++)
                    inputAsBlocks[j][c][r] = (inputAsBlocks[j][c][r] - roundKeys[rounds*3-i*3+c][r] + 37) % 37; //Add 37 to ensure output is positive
        ...
        for (int j = 0; j < inputAsBlocks.size(); j++)
        {
            ...
            inputAsBlocks[j][0][1] = inputAsBlocks[j][2][1];
            inputAsBlocks[j][2][1] = inputAsBlocks[j][1][1];
            inputAsBlocks[j][1][1] = temp;
            
            temp = inputAsBlocks[j][0][2];
            inputAsBlocks[j][0][2] = inputAsBlocks[j][1][2];
            inputAsBlocks[j][1][2] = inputAsBlocks[j][2][2];
            inputAsBlocks[j][2][2] = temp;
        }
        ...
        for (int j = 0; j < inputAsBlocks.size(); j++)
            for (int c = 0; c < 3; c++)
                for (int r = 0; r < 3; r++)
                    inputAsBlocks[j][c][r] = (20 * ((inputAsBlocks[j][c][r]+20)%37)) % 37;
    }
    ...
    for (int i = 0; i < inputAsBlocks.size(); i++)
        for (int c = 0; c < 3; c++)
            for (int r = 0; r < 3; r++)
                inputAsBlocks[i][c][r] = (inputAsBlocks[i][c][r] - roundKeys[c][r] + 37) % 37;
    ...
    for (int i = 0; i < inputAsBlocks.size(); i++)
        for (int c = 0; c < 3; c++)
            for (int r = 0; r < 3; r++)
                output += reference[inputAsBlocks[i][c][r]];
